...
public class Win32SecurityLibrary {
  ...
  public static SECURITY_ATTRIBUTES createSecurityWithLogonDacl(int accessMask) {
    return createSecurityWithDacl(accessMask, Win32NamedPipeLibraryProvider.LOGON_DACL);
  }

  public static SECURITY_ATTRIBUTES createSecurityWithDacl(int accessMask, int securityLevel) {
    if (securityLevel == Win32NamedPipeLibraryProvider.NO_SECURITY) return null;
    ...
    if (securityLevel == Win32NamedPipeLibraryProvider.LOGON_DACL) getLogonSID(psid);
  ...
  public static void getOwnerSID(PSIDByReference psid) {
    ...
    try {
      Advapi32.INSTANCE.OpenProcessToken(
          Kernel32.INSTANCE.GetCurrentProcess(), WinNT.TOKEN_QUERY, phToken);
      Native.getLastError();
