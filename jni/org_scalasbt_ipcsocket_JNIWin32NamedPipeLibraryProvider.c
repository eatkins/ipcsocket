...
static int createSecurityWithDacl(PSECURITY_ATTRIBUTES pSA,
                                       DWORD accessMask, BOOL logon);
...
Java_org_scalasbt_ipcsocket_JNIWin32NamedPipeLibraryProvider_CreateNamedPipeNative(
    ...
    jint nIntBufferSize, jint nDefaultTimeout, jint lpSecurityAttributes, jint security) {
  PSECURITY_ATTRIBUTES pSA = security ? HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY,
                                       sizeof(SECURITY_ATTRIBUTES)) : NULL;
  int err = security ? createSecurityWithDacl(pSA, lpSecurityAttributes, security == 1) : 0;
  if (!err || !security) {
    ...
    if (pSA) HeapFree(GetProcessHeap(), 0, pSA);
    return handle;
  } else {
    char buf[512];
...
static int createSecurityWithDacl(PSECURITY_ATTRIBUTES pSA,
  ...
  if (logon) {
    ...
    groups = (PTOKEN_GROUPS)HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY,
    ...
    owner = (PTOKEN_OWNER)HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY,
				 tokenInformationLength * sizeof(TOKEN_OWNER));
    GetTokenInformation(phToken, TokenOwner, owner, tokenInformationLength, &tokenInformationLength);
    psid = owner->Owner;
  }
  if (psid == NULL)
    goto psidfail;
